...
export default function Signup() {
  ...
  const validate = () => {
    ...
    else if (!/[A-Z]/.test(formData.password)) newErrors.password = t("passwordUpper");
    else if (!/[a-z]/.test(formData.password)) newErrors.password = t("passwordLower");
    else if (!/[0-9]/.test(formData.password)) newErrors.password = t("passwordNumber");
    else if (!/[!@#$%^&*]/.test(formData.password)) newErrors.password = t("passwordSpecial");
      ...
      <div className="h-full flex flex-wrap items-center justify-center lg:justify-between">
        ...
        <div className="mb-12 md:mb-0 md:w-8/12 lg:w-5/12 xl:w-5/12">
          <form onSubmit={handleSubmit}>
            ...
            {errors.terms && <p className="mb-4 text-red-600 font-semibold">{errors.terms}</p>}

            {/* Server / Success Message */}
            {message && <p className="mb-4 text-green-600 font-semibold">{message}</p>}
            {errors.server && <p className="mb-4 text-red-600 font-semibold">{errors.server}</p>}
            ...
            <div className="text-center lg:text-left">
              ...
              <p className="mb-0 mt-2 pt-1 text-sm font-semibold">
                {t("haveAccount")}{" "}
                <Link to="/login" className="text-red-600 hover:text-red-700">{t("login")}</Link>
              </p>
            </div>
          </form>
        </div>
